...
---------------------------
The first approach we're going to take is to define a function that transforms your data into a tree of HTML elements.

The Document Object Model, or DOM, is a programmatic interface to your HTML document. With DOM methods, you can dynamically create elements from JavaScript.
...
-----------------------
One potential downside to using DOM methods to generate HTML is the verbosity of the DOM APIs. If you have a lot of elements to create, the code can become a bit tedious.
...
As you can see, though, creating strings of HTML in JavaScript is ugly and very error-prone. It's very easy to generate invalid HTML by forgeting or misplacing a single or double quote. I don't recommend building HTML strings in JavaScript for any project.

Using a Template Engine
-----------------------
A third way to generate HTML from your JavaScript objects is to use a template engine such as [Mustache] or [Handlebars]. Templates allow you to define the structure of your HTML in a way that looks very close to the final rendered HTML. The template engine is then responsible for merging the template and the JavaScript content. In the example below, we are going to use the [Handlebars] template engine.

The first step is to define your template. Handlebars templates look like regular HTML with some expressions that are wrapped in double curly quotes like `{{ }}`:
...
Using a template engine helps improve the readability and organization of your project, but it's not without its costs. Dynamically compiling templates takes some time, and we add the overhead of an extra JavaScript file just for templating.
...
When you are manipulating the DOM from JavaScript, it's generally better to batch your changes than to make a lot of small changes. In the examples above, we touch the DOM once for every entry in our bookmarks list.
